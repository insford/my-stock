...
* `guide/data/live_market.js`는 **10분 주기 GitHub Actions 워크플로(`monitor.yml`)가 자동으로 갱신·커밋·푸시**하는 파일입니다. 이 파일을 직접 편집하지 말고, 작업 시작 전 `git status` / `git log -3`으로 Actions 자동 커밋 또는 다른 세션(특히 antigravity-cli)의 미반영 변경사항이 있는지 먼저 확인한 뒤 `git pull`로 최신 상태를 받습니다.
