...
flowchart TD
    subgraph Data_Pipeline["1. 데이터 수집 & SQLite 적재 (Python 배치)"]
        CRON["GitHub Actions (30분 주기 monitor.yml)"] --> UPDATER["update_history.py --update"]
...
2. **일일 갱신 모드 (`--update`, 30분 주기 통합 자동 실행)**:
   * 각 종목별 최근 3일치(`pageSize=3`) 조회
   * 기존 DB에 `INSERT OR REPLACE` 실행 → 최근 3일 치 종가 갱신 및 신규 날짜 자동 추가
